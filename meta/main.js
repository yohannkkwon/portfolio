...
function processCommits(data) {
  return d3
    ...
    .map(([commit, lines]) => {
      ...
      let ret = {
        id: commit,
        url: 'https://github.com/yohannkkwon/portfolio/commit/' + commit,
        author,
        date,
        time,
        timezone,
...
function renderCommitInfo(data, commits) {
  // Clear out anything that was there before so this is idempotent
  ...
  if (longest) {
    ...
    dl.append('dd').text(`${longest[0]} (${longest[1]})`);
  }
...
function updateScatterPlot(data, commits) {
  ...
  // Clear out the existing x-axis and redraw
...
function onTimeSliderChange() {
  ...
  renderCommitInfo(
    data.filter((d) => new Date(d.datetime) <= commitMaxTime),
    filteredCommits,
  );
...
function onStepEnter(response) {
  ...
  // Sync the slider position with the scroll position
